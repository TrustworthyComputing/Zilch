; ./stark-tinyram examples-tinyram/knowledge_of_bignum_factorization.asm -t14 -s120 -P./examples-tinyram/knowledge_of_bignum_factorization.pubtape -A./examples-tinyram/knowledge_of_bignum_factorization.auxtape

; Initialize memory to store blocks of product with zero
MOV r1 r1 0
__init_mult_mem__
ADD r2 r1 3000                      ; calc index to store it
STOREW r0 r0 r2                     ; init mem with zero
ADD r1 r1 1                         ; i++
CMPE r0 r1 42                       ; flag = (r1 == 42)
CNJMP r0 r0 __init_mult_mem__    ; if (!flag) then jump to __init_mult_mem__
...
CNJMP r0 r0 __read_p__   ; if (!flag) then jump to __read_p__
...
ADD r1 r1 1                 ; i++
CMPE r0 r1 21               ; flag = (r1 == 21)
CNJMP r0 r0 __read_q__   ; if (!flag) then jump to __read_q__
...
AND r0 r8 255           ; lo = mult & ((1<<BITS_PER_BLOCK) - 1)
SHR r1 r8 8             ; hi = (mult >> BITS_PER_BLOCK)

ADD r12 r3 r5           ; i + j
ADD r12 r12 3000        ; i + j + 3000 // calc index to store it
; num_blocks[i + j] += hi
LOADW r10 r10 r12       ; load num_blocks[i + j]
ADD r10 r10 r1          ; num_blocks[i + j] += hi
STOREW r10 r10 r12      ; store num_blocks[i + j]
; num_blocks[i + j + 1] += lo
ADD r12 r12 1           ; i + j + 1 + 3000 // calc next index 
LOADW r10 r10 r12       ; load num_blocks[i + j + 1]
ADD r10 r10 r0          ; num_blocks[i + j + 1] += lo
STOREW r10 r10 r12      ; store num_blocks[i + j + 1]

ADD r4 r4 1                 ; jj++ 
CMPE r0 r4 21               ; flag = (jj == 21) 
CNJMP r0 r0 __for_all_qi__   ; if (!flag) then jump to __for_all_qi__

ADD r2 r2 1                 ; ii++ 
CMPE r0 r2 21               ; flag = (ii == 21) 
CNJMP r0 r0 __for_all_pi__   ; if (!flag) then jump to __for_all_pi__


MOV r1 r1 41                        ; i = (2 * 21) - 1
__check_overflows__
ADD r2 r1 3000                      ; calc index to load from
LOADW r10 r10 r2                    ; load num_blocks[i]
SUB r1 r1 1                         ; i--

CMPG r10 r10 255                    ; flag = ( num_blocks[i] > ((1 << BITS_PER_BLOCK)-1) )
CNJMP r0 r0 __no_overflow__         ; if (!flag) then jump to __no_overflow__
...
CMPE r0 r4 r10                      ; and check if it is equal with the block multiplication

CNJMP r0 r0 __not_equal__            ; if (r4 != r10) then jump to __not_equal__

CMPE r1 r1 1                            ; flag = (r1 == 1)
CNJMP r0 r0 __check_overflows__      ; if (!flag) then jump to __check_overflows__
