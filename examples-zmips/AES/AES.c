...
void inverseMixedColumn(uint8_t* ptxt) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        tmp[(4*i)+1] = (uint8_t) (mul_9[ptxt[(4*i)+0]] ^ mul_14[ptxt[(4*i)+1]] ^ mul_11[ptxt[(4*i)+2]] ^ mul_13[ptxt[(4*i)+3]]);
        tmp[(4*i)+2] = (uint8_t) (mul_13[ptxt[(4*i)+0]] ^ mul_9[ptxt[(4*i)+1]] ^ mul_14[ptxt[(4*i)+2]] ^ mul_11[ptxt[(4*i)+3]]);
        tmp[(4*i)+3] = (uint8_t) (mul_11[ptxt[(4*i)+0]] ^ mul_13[ptxt[(4*i)+1]] ^ mul_9[ptxt[(4*i)+2]] ^ mul_14[ptxt[(4*i)+3]]);
    }
    for (int i = 0; i < 16; ++i) {
        ptxt[i] = tmp[i];
    }
...
void inverseByteSubShiftRow(uint8_t* ptxt) {
    uint8_t tmp[16];
    tmp[0] = inv_s[ptxt[0]];
    tmp[1] = inv_s[ptxt[13]];
    tmp[2] = inv_s[ptxt[10]];
    ...
    tmp[9] = inv_s[ptxt[5]];
    tmp[10] = inv_s[ptxt[2]];
    tmp[11] = inv_s[ptxt[15]];
    tmp[12] = inv_s[ptxt[12]];
    tmp[13] = inv_s[ptxt[9]];
    tmp[14] = inv_s[ptxt[6]];
    tmp[15] = inv_s[ptxt[3]];
    for (int i = 0; i < 16; ++i) {
...
void AESEncryption(uint8_t* ptxt, uint8_t* expandedKey, uint8_t* ctxt) {
    ...
    byteSubShiftRow(state);
    for (int i=0 ; i<16 ; i++) {
        ctxt[i] = state[i] ^ expandedKey[i+160];
    }
    free(state);
}

void AESDecryption(uint8_t* ctxt, uint8_t* expandedKey, uint8_t* ptxt) {
    uint8_t* state = malloc(16);
    // key whitening
    for (int i = 0; i < 16; ++i) {
        state[i] = ctxt[i] ^ expandedKey[160+i];
    }
